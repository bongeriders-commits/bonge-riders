...
// ---- Pull-to-refresh ----
// Only arms when the feed is scrolled all the way to the top, so it never
// fights with normal scrolling through the feed. A small drag indicator
// tracks the pull distance; releasing past the threshold triggers loadFeed().
...
feedEl.addEventListener('touchmove', (e) => {
  ...
  const pull = Math.min(delta, PULL_THRESHOLD * 1.5);
  indicator.style.opacity = Math.min(pull / PULL_THRESHOLD, 1);
  indicator.style.transform = `translateY(${pull}px) rotate(${pull >= PULL_THRESHOLD ? 180 : 0}deg)`;
  indicator.textContent = pull >= PULL_THRESHOLD ? '↑' : '↓';
}, { passive: true });
...
async function loadFeed() {
  feedEl.innerHTML = '';
  pullIndicator = null; // old node was just wiped out with the feed
  lastDoc = null;
  reachedEnd = false;
  usingFiller = false;
  fillerPage = 1;
...
function renderSlide(id, data, insertBefore, isFiller) {
  ...
  // Single tap toggles play/pause; a second tap landing within 300ms in
  // roughly the same spot counts as a double-tap and likes the veed
  // instead (TikTok-style) — the pending single-tap action is cancelled.
  let lastTapTime = 0;
  let lastTapX = 0;
  let lastTapY = 0;
  ...
  slide.addEventListener('click', (e) => {
    ...
    if (isDoubleTap) {
      ...
      lastTapTime = 0;
      likeFromDoubleTap(id, slide, e.clientX, e.clientY);
      return;
    }
...
// Double-tap always likes (never unlikes), matching the TikTok convention —
// tapping an already-liked veed again just replays the heart animation
// without touching the like count a second time.
async function likeFromDoubleTap(videoId, slide, clientX, clientY) {
  spawnHeartBurst(slide, clientX, clientY);
  if (!currentUser) return; // still show the animation, just skip the write for guests
  const alreadyLiked = slide.dataset.filler === 'true'
    ? slide.querySelector('.like-btn').classList.contains('liked')
    : likedIds.has(videoId);
  if (!alreadyLiked) {
    await toggleLike(videoId, slide);
  }
}

function spawnHeartBurst(slide, clientX, clientY) {
